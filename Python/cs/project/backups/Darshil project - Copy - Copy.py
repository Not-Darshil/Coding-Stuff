...
while True:
    try:
        ...
        dbobj=con.connect(host="localhost",user="root",password=pswd,
...
def dbtable():
    ...
    crsr.execute("create table if not exists product(PNO int,PNAME char(15),BRAND char(15),MRP int,SELLINGP int)")
        ...
        for w in range (n):
            try:
                ...
                if confirm=="y" or confirm=="Y":
                    crsr.execute("insert into product values {}".format(rec))
                    crsr.execute("commit")
                    print()
        ...
        try:
            ...
            if no==0:
                ...
                for i in range(len(pnolist)):
                    ...
                    print("%3s  %-15s %-15s %4s  %-3s  %-5s %5s %5s"%(rec[0][0],rec[1][0],rec[2][0],rec[3][0],"QTY","PRICE","DISC","COST"))
